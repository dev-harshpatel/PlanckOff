...
This document is grounded in the **current repository** as of the analysis date. File paths, exports, and behaviors refer to code under `D:\Projects\learning-projects\PlanckOff` unless noted.
...
| **`docs/AI_PIPELINE_DOCUMENTATION.md`** | States match is “deprecated” and emphasizes unified flow + local JSON; **current product UI** uses **`ImportFilesModal`** → sequential **`/api/extract`** → **`/api/match`** → **`/api/finalize`** per **`docs/APPLICATION_OVERVIEW.md`** and **`src/components/features/project/ImportFilesModal.tsx`**. Treat AI doc as partially stale for “default path”. |
...
- [x] **`material-database.json`** — **461** top-level array entries; stringified size ~**265 KB** (Node check). Fields include **`code`**, **`section`**, **`matCost`**, **`per`**, **`category`**, **`description`**, etc. (see file header rows).
