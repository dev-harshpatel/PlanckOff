...
export const DELETE = withRoleAuth(
  ...
  async (request, { user, teamMember: currentUser }, params) => {
    try {
      ...
      const { data: memberToDelete, error: memberError } = await getTeamMemberById(id);
      ...
      if (currentUserLevel >= memberLevel) {
        return NextResponse.json(
          { success: false, error: 'You do not have permission to delete this member' },
          { status: 403 }
        );
      }
...
export const PUT = withRoleAuth(
  ...
  async (request, { user, teamMember: currentUser }, params) => {
    try {
      ...
      const { data: memberToUpdate, error: memberError } = await getTeamMemberById(id);
      ...
      if (!isSelf && currentUserLevel >= memberLevel) {
        return NextResponse.json(
          { success: false, error: 'You do not have permission to update this member' },
          { status: 403 }
        );
      }

      // Build update object
      const updates: { name?: string; email?: string; role_id?: string; status?: string } = {};
        ...
        if (role && typeof role === 'string') {
          ...
          if (roleData) {
            ...
            if (newRoleLevel <= currentUserLevel) {
              return NextResponse.json(
                { success: false, error: 'You cannot assign a role equal to or higher than your own' },
                { status: 403 }
              );
            }
      ...
      // Perform update
      const { data: updatedMember, error: updateError } = await updateTeamMember(id, updates);
