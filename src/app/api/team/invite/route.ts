...
import { sendInvitationEmail, generateInvitationUrl } from '@/lib/email/invitation';
  ...
  async (request, { user, teamMember }) => {
    try {
      ...
      if (!canInviteRole(user.role, role)) {
        return NextResponse.json(
          { success: false, error: `You don't have permission to invite ${role}s` },
          { status: 403 }
        );
      }
      ...
      const { data: existingInvitations } = await getPendingInvitationsByEmail(email);
