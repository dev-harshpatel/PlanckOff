...
export function applyProjectWasteOverrides(
  ...
  const updated = costingData.map((assembly) => ({
    ...
    materials_costing: assembly.materials_costing.map((item) => ({
      ...item,
      matched_material: (() => {
        const material = item.matched_material;
        if (material == null) return material;
        const wastePercent = overrideMap[material.code]?.wastePercent;
        if (wastePercent == null || material.waste_percent === wastePercent) return material;
        changed = true;
        return { ...material, waste_percent: wastePercent };
      })(),
  ...
  const updated = costingData.map((assembly) => ({
    ...
    materials_costing: assembly.materials_costing.map((item) => ({
      ...
      matched_material: (() => {
        ...
        if (unitCost == null || material.unit_cost === unitCost) return material;
        changed = true;
        return { ...material, unit_cost: unitCost };
      })(),
      matched_labor: (item.matched_labor ?? []).map((labor) => {
