...
import { Mail, MoreHorizontal, Plus, Shield, User, Users, X, Check, Trash2, Edit2 } from 'lucide-react';
  ...
  const openModal = (member?: TeamMember) => {
    if (member) {
      ...
      setFormData({
        name: member.name,
        email: member.email,
        role: member.role || 'Estimator', // Default fallback
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          ...
          >
            <Plus className="w-4 h-4" /> Invite New
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50">
              <h3 className="font-bold text-slate-800">{editingMember ? 'Edit Team Member' : 'Add New Member'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-2 text-slate-500 font-medium hover:bg-slate-50 rounded-lg">Cancel</button>
                <button type="submit" className="flex-1 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 shadow-sm flex items-center justify-center gap-2">
