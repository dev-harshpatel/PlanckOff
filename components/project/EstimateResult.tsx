...
import { ConditionDetailModal } from '@/components/project/ConditionDetailModal';
import { v4 as uuidv4 } from 'uuid';
import { DEFAULT_TEMPLATES, AssemblyTemplate } from '@/constants/defaultAssemblies';
import { AssemblyEditorModal } from '@/components/project/AssemblyEditorModal';
import { TakeoffScheduleView } from '@/components/project/TakeoffScheduleView';
import { AssemblySummaryGrid } from '@/components/project/AssemblySummaryGrid';
...
import { Button, IconButton, SearchInput, Modal } from '@/components/common';
...
export const EstimateResult: React.FC<EstimateResultProps> = ({
    ...
    const priceMap = useMemo(() => {
        const map: Record<string, { cost: number, per?: string, waste?: number, supplier?: string }> = {};
        materials.forEach(m => {
    ...
    const convertMat = (mat: CalculatedMaterial) => {
        if (displayUnit === 'imperial') {
            ...
            if (pricing) {
                ...
                totalCost = pricingQty * (pricing.cost / (pricing.per ? parsePer(pricing.per) : 1));
            }
