    ...
    const handleUpdateMaterial = (code: string, field: keyof MaterialDefinition, value: any) => {
        const updated = materials.map(m => {
            if (m.code === code) {
                ...
                // Reactive Calc for Labor
                if (m.category === 'Labor') {
                    if (field === 'hourlyRate' || field === 'productivity') {
                        const r = field === 'hourlyRate' ? value : newItem.hourlyRate; // Use newItem.hourlyRate for current value
                        const p = field === 'productivity' ? value : newItem.productivity; // Use newItem.productivity for current value
                        if (r && p) {
                    ...
                    // Optional: If Cost changed, update Rate? or Prod? (Let's keep uni-directional for consistency: Rate + Prod -> Cost)
                }
                return newItem;
            }
            return m;
        ...
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={handleExportDatabase} className="px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 flex items-center gap-2 shadow-sm transition-colors">
                        <Download className="w-4 h-4" /> Export
                    </button>
                    <label className="px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 flex items-center gap-2 shadow-sm cursor-pointer transition-colors">
                    ...
                    }} className="px-4 py-2 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg text-sm font-medium hover:bg-blue-100 flex items-center gap-2 shadow-sm transition-colors">
                        <Database className="w-4 h-4" /> Calc Prod
                    </button>
                </div>
            </div>
                ...
                <div className="flex-1 flex flex-col bg-white">
                    ...
                    <div className="p-4 border-b border-slate-200 flex gap-4 items-center">
                        <div className="relative flex-1">
                            ...
                            <input
                                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                                placeholder="Search by code, description, or manufacturer..."
                                value={dbSearch}
                                onChange={(e) => setDbSearch(e.target.value)}
                            />
                        </div>
                        <button onClick={() => setIsAddingMat(true)} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg shadow-sm flex items-center gap-2 transition-colors">
                                ...
                                {isAddingMat && (
                                    <tr className="bg-blue-50 animate-in fade-in duration-300">
                                        ...
                                        <td className="px-4 py-2 text-right flex gap-1 justify-end">
                                            <button onClick={handleAddMaterial} className="text-green-600 hover:bg-green-100 p-1.5 rounded transition-colors"><Save className="w-4 h-4" /></button>
                                            <button onClick={() => setIsAddingMat(false)} className="text-slate-400 hover:bg-slate-100 p-1.5 rounded transition-colors"><X className="w-4 h-4" /></button>
                                        </td>
                                    </tr>
                                )}
                                {filteredDbMaterials.map(m => (
                                    <tr key={m.code} className="hover:bg-slate-50 group transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <button onClick={() => handleDeleteMaterial(m.code)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all p-1 rounded hover:bg-slate-100">
