...
import { FORMULA_DEFINITIONS } from './constants'; // Adjust path if needed
...
export const getRowDetails = (comp: AssemblyComponent, assembly: WallAssembly, instances: TakeoffInstance[]) => {
    ...
    const getWaste = (c: AssemblyComponent, defaultCategory: string) => (c.wasteFactor !== undefined && c.wasteFactor !== null) ? c.wasteFactor : (DEFAULT_WASTE_FACTORS[defaultCategory] ?? 0.05);
