...
import { WallAssembly, CalculatedMaterial, TakeoffInstance, AssemblyComponent, CalculationMethod, MaterialDefinition } from "../types";

// Lazy initialization of the AI client to prevent app crash if API key is missing
const getAIClient = () => {
    // try Vite env first, then process.env
    const key = (import.meta as any).env?.VITE_API_KEY || process.env.API_KEY;
    if (!key) {
        throw new Error("API_KEY environment variable is not set. Please set VITE_API_KEY in your .env file.");
    }
...
const RECIPES = {
    framing: {
        metal: {
            labor: { name: 'Journeyman Carpenter (Framing/Drywall)', rate: 0.020, unit: 'HR', per: 'sqft' }, // Slightly increased for bracing
            screw: { name: '7/16" Wafer Head Framing Screws', rate: 0.004, unit: 'box (1000)', per: 'sqft' },
            pin: { name: '1" Drive Pins (Concrete/Steel)', rate: 1.0, unit: 'box (100)', per: 'lf' }, // logic is dynamic
            channel: { name: '1-1/2" Cold Rolled Channel', rate: 0.25, unit: '16\' pcs', per: 'sf' }, // logic is dynamic
...
export const evaluateMath = (expression: string, vars: Record<string, number>): number => {
    // Do not convert the whole expression to lower case, as it breaks function calls like Math.round().
    ...
    try {
        ...
        // Ensure the evaluated result is a valid number before returning.
...
export const identifyWallAssemblies = async (
    base64Image: string,
    mimeType: string,
    ...
    // FIX: Replaced explicit `Schema` import with `any` to align with guideline preferences against similar types like `SchemaType`.
                        ...
                        components: {
                            ...
                            items: {
                                ...
                                properties: {
                                    ...
                                    category: { type: Type.STRING, enum: ["Framing", "Cladding", "Insulation", "Accessory"] },
  ...
  `;
    ...
    try {
        ...
        throw new Error(`Failed to identify wall types: ${error.message || String(error)}`);
    }
};
...
export const calculateMaterials = (
    ...
    const addDependencies = (cat: string, qty: number, skipLabor: boolean = false) => {
        ...
        if (cat === 'Framing') {
            ...
            if (framingType !== 'Wood') {
                ...
                const pinQty = Math.ceil(totalLinearFeet * 2 / 2); // 1 pin every 2ft, top and bottom
    ...
    assembly.components.forEach(comp => {
        ...
        let compSect = '09250'; // Default section (Drywall)
        ...
        switch (usage) {
            ...
            case 'Fastener (per SqFt)':
                category = 'Finishing';
                const screwCount = Math.ceil((totalWallArea || totalCeilingArea) * 2.2); // Approx 2.2 screws/sf
                quantity = Math.ceil(screwCount / 1000);
                unit = 'box';

                // Length Logic
                const minPenetration = 0.625; // 5/8"
                ...
                // Type Logic based on Framing Type
                ...
                // Find specific screw in database
            ...
            case 'Fixed Qty':
                ...
                break;
            default:
                // Dynamic Spacing Handler (e.g. "Vertical @ 18" OC")
        ...
        if (quantity > 0) {
            ...
            // OST-Style Component Labor
            if (comp.installRate && comp.installRate > 0) {
                const laborQty = Math.ceil(quantity * comp.installRate);
                if (laborQty > 0) {
                    mats.push({
                        category: 'Labor',
                        item: 'Journeyman Carpenter (Framing/Drywall)', // Maps to DB Rate
                        quantity: laborQty,
                        unit: 'HR', // Should be HR
                        notes: `Install ${finalItemName}`,
                        overridePrice: 65, // Default Rate
                        grade: 'Labor',
                        sect: compSect,
        ...
        if (triggerDependencyCategory && dependencyQuantity > 0) {
            // Skip generic labor if specific labor is defined for this component
