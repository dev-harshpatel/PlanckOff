...
interface AppContextValue {
  ...
  setMaterials: (value: MaterialDefinition[] | ((prev: MaterialDefinition[]) => MaterialDefinition[])) => void;

  // Role Permissions
  rolePermissions: Record<UserRole, string[]>;
  setRolePermissions: (value: Record<UserRole, string[]> | ((prev: Record<UserRole, string[]>) => Record<UserRole, string[]>)) => void;

  // Default Assemblies
  defaultAssemblies: AssemblyTemplate[];
  setDefaultAssemblies: (value: AssemblyTemplate[] | ((prev: AssemblyTemplate[]) => AssemblyTemplate[])) => void;
...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const [materials, setMaterials] = useLocalStorage<MaterialDefinition[]>(
    'drywallSpec_materials_v5',
    DEFAULT_CATALOG
  );

  // Persistent Role Permissions
  const [rolePermissions, setRolePermissions] = useLocalStorage<Record<UserRole, string[]>>(
    'drywallSpec_roles_v1',
    DEFAULT_ROLE_PERMISSIONS
  );

  // Persistent Default Assemblies
  const [defaultAssemblies, setDefaultAssemblies] = useLocalStorage<AssemblyTemplate[]>(
    'drywallSpec_default_assemblies_v1',
    DEFAULT_TEMPLATES
  );
